...
3. My knowledge of DST mechanics is INCOMPLETE — I've been reverse-engineering from scripts.zip and asking you. I don't reliably know: day length (how many real seconds), dusk/night timing, food values, frog aggression radius, seasonal threats (deerclops/hounds day ranges), sanity mechanics, kiting behavior, etc.

## Knowledge sources — which should I build on?
1. **Game scripts** (`<game>/data/databundles/scripts.zip`, ~4000 .lua files) — ground truth for APIs/mechanics. I read these directly (zipfile + Python). Always available offline. BUT: reverse-engineering facts (timings, damage numbers, spawn tables) from Lua is slow and error-prone.
2. **dontstarve.wiki.gg** — MediaWiki API (`/api.php?action=parse&page=X&format=json&prop=wikitext`) returns structured recipe/damage/behavior facts (e.g. Axe = 1 Twigs + 1 Flint, damage 27.2, 100 uses). Works but rate-limits (403 after 1-2 requests — needs throttle/retry/backoff).
3. **dontstarve.fandom.com** — blocks direct requests (403 always).
4. **Klei official site** — works (HTML), but mostly marketing/overview, not detailed mechanics.

## My questions for you
1. **Which source should be the PRIMARY knowledge base for a bot like this?** My lean: scripts.zip for API mechanics + wiki.gg (throttled) for game facts (recipes, timings, mob stats, food values). Is that right? Is there a better source I'm missing (e.g. a machine-readable data dump, klei forums data, a JSON/game-data extraction)?
2. **How should the bot STORE and USE this knowledge?** Options:
   a. A static facts file (JSON/MD) I hand-build from wiki+scripts: recipes, day length, mob stats, food values, seasonal events. Loaded into my context each session.
   b. A RAG-style lookup: fetch wiki pages on-demand when the situation needs it.
   c. Teach the mod to EXPOSE more game state (mob distances, day-time-remaining, nearby threat types) so I don't need wiki knowledge — the game tells me everything through state.json.
   My instinct: (c) first (expose game state via mod), (a) for static facts, wiki lookups only for things the game can't expose.
3. **What are the TOP ~20 facts a survival AI needs that I should hard-code?** (day length in seconds, dusk/night durations, hunger drain rate, food values, frog/spider/hound aggression, deerclops arrival day, winter timing, sanity drain at night, etc.) List them with approximate values so I can verify against wiki/scripts and build the facts file.
4. **Architecture**: reflex daemon (flee/eat, 200ms) + me as deliberative layer (decide goals/actions, ~1-5s cadence) + job runner. Any refinement now that the basics work? What state would make my decisions 10x better (e.g. time-until-dusk in seconds, nearest-threat distance, current biome, available recipes, workleft of target)?

## NEW FEATURE IDEA: Wilson personality/dialogue (user's request)
The user wants Wilson to be FUN to watch, especially when he's idle waiting for commands. Ideas:
- Wilson says contextual lines: when hungry ("I could really use some berries..."), when idle ("So... what's the plan, boss?"), when gathering ("These twigs will make a fine axe!"), when scared of frogs ("THOSE FROGS! WHY!?"), at dusk ("Getting dark... I don't want to be caught out here!").
- The mod currently has a working `say` command (talker:Say -> speech bubble). 
- Question: what's the cleanest way to add this? (a) mod-side idle dialogue timers (pick from a line table based on state), (b) brain-side: I send a say command alongside each action, (c) both. Also: how often should Wilson talk so it's fun but not spammy? Any DST modding gotchas with talker:Say (cooldowns, text length, client/server)?

## Constraints
- Windows, DST via Steam, offline solo host, user watches in real-time
- No computer vision (text-only sensing via mod)
- Token economy matters (I re-read state every ~2s)
- Must work offline (no cloud wiki dependency at runtime)

---

# THE FULL MOD FILE (modmain.lua) — please review and suggest upgrades
