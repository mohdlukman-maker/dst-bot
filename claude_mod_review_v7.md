# DST AI Bot — Claude Review Request (v7): Mod is 72KB / 1568 lines, needs a full audit

## ⚠️ CONTEXT: the last two bugs were SCOPE bugs in write_state that killed the state file silently

1. **The classify closure** (per-entity scan classifier) threw on some entity → write_state died every tick → state file NEVER written → bot blind. Fixed: per-entity PCALL isolation.
2. **The "blind" dialogue hook** referenced `best[n]` — but `best` was a local inside the scan block, OUT OF SCOPE at the blind check → `attempt to index global 'best' (a nil value)` at line 643 → write_state died AGAIN. Fixed: hoisted `local best = nil` + `if best then` guard.
3. Added: `poll()` now LOGS write_state errors (`[DST-AI-BOT] write_state ERROR: ...`) so this class can never be silent again.

**The lesson: any closure/hook that references a variable from a nested block is a landmine. Please hunt for ALL of these.**

## What to audit (ranked)

### A. SCOPE BUGS (the killer class — check EVERY closure)
- Every `local function` / closure defined inside write_state or execute_command that references variables from OUTER blocks — verify those variables are in scope (declared before, same or outer scope)
- Every `PCALL(function() ... end)` closure — does it capture locals that exist?
- The `classify` closure — verify `best`, `seen`, `px/py/pz`, `nearby` all in scope
- The blind-check block — verify `scan_dropped`, `best` guards
- `wilson_event`/`pick_line`/`wilson_say` — verify they're declared BEFORE all callers (Lua upvalue ordering)
- `equip_best_tool`/`best_tool_for` — declared before job_start?
- Any `heartbeat_tick_last_paused`, `blind_said` — global-vs-local declarations

### B. LOAD-TIME CRASH RISKS
- Odd quotes, `break` in wrong place, colon-in-condition (`if e:HasTag` style)
- `e2 = e3 break` patterns (Lua 5.1 allows break-as-last-statement)
- Unbalanced blocks (my checker says NONE, but verify)

### C. RUNTIME CRASH RISKS
- `json.encode` on tables with non-encodable values (functions, circular refs) — anything in `st` that could break encode
- `TheSimRef:FindEntities` args (radius, must_have, cant_have, oneof — verify the nil positions)
- `myplayer.components.*` nil guards on ALL component accesses
- `inv:Has(nm, 1, true)` — 3-arg checkallcontainers semantics

### D. THE REMAINING MYSTERIES (need your eyes)
1. **Inventory reporting**: state showed `items: []` / `equipped: []` while vision confirmed Wilson holding an axe. Was this ALSO the write_state crash (state never written → I read stale/absent), or is there a real inventory-reading bug? The debug_inventory command now exists — but I need the theory before running.
2. **Gather phantom-gain**: jobs reported `gained: {flint:1}` but inventory empty — was this also the state-write crash poisoning my reads, or a real sweep bug?
3. **Pick-timing**: I added a 1200ms wait after PICK before settling (was settling after 0.25s = one tick). Is 1200ms right? Or should I use `DoAction` completion events?
4. **The `work` field**: nearby entries have `work = tostring(wa)` where wa is `workable.action` — is that a table (action object)? `tostring` on it gives "table: 0x..." — harmless but noisy. Better: `wa.id` or `wa == ACTIONS.CHOP and "CHOP"`?

## Key architecture reminders
- Offline solo host, ismastersim=true (we ARE the server)
- State file: `dst_ai_bot_state` (persistent string, KLEI format)
- Commands: move_to, gather_job (job-runner w/ busy-gate + sweep + gained-delta verify), craft (DoBuild w/ pt for placers), fuel, deploy, equip, eat, attack, revive (playerghost), say, set_dialogue, debug_inventory, dump_knowledge, preempt_job
- Reflex daemon (Python): flee hostiles, eat<30, dusk-90s light, fire-fuel, freezing→fire
- Recent fixes: heartbeat (DoStaticPeriodicTask + IsServerPaused), scan audit (st.scan dropped counts), unfiltered sensing + component classify, best_tool_for by effectiveness + light guard, Wilson dialogue (15 categories, event hooks)
    ...
    do
        ...
        if TheSimRef and TheSimRef.FindEntities then
            ...
            if ents and type(ents) == "table" then
                ...
                local classify = function(e)
                    ...
                    if e.components then
                        ...
                        if e.components.workable then
                            ...
                            if okwa and wa then entry.work = tostring(wa) end
                        end
                ...
                -- SCAN AUDIT: what did we see vs what did we keep? (meta-fix)
                -- dropped = seen prefabs that did NOT make the nearby list
                -- (e.g. rain FX, distant entities, or - the bug class - something filtered)
                for n, cnt in pairs(seen) do
                    ...
                    if kept < cnt then
                        scan_dropped[n] = cnt - kept
                    end
                end
            end
        end
        st.nearby = nearby
        st.scan = { total = 0, kept = #nearby, dropped = scan_dropped }
        ...
        if TheSimRef and TheSimRef.FindEntities then
            local _, allents = PCALL(function()
                return TheSimRef:FindEntities(px, py, pz, 30, nil, nil, nil)
            end)
            if allents and type(allents) == "table" then
                st.scan.total = #allents
            end
        end
    end
...
local function job_tick()
    ...
    if job.mode == "pick" then
        ...
        elseif (TheSimRef:GetRealTime() - (job.pick_pushed_at or 0)) < 1200 then
            return  -- wait for the pick animation to actually complete
        else
            job.phase = "settling"
            job.work_done_at = TheSimRef:GetRealTime()
        end
...
local function equip_best_tool(workaction)
    ...
    local was_wrong = false
    if hand and not (hand.components and hand.components.tool and
        (PCALL(function() return hand.components.tool:GetEffectiveness(workaction) end) or 0) > 0) then
        was_wrong = true
    end
...
local function execute_command(cmd)
    ...
    elseif action == "debug_inventory" then
        ...
        local info = {
            itemslots_count = (inv.itemslots and #inv.itemslots) or "nil",
            itemslots_keys = {},
            equipslots_count = (inv.equipslots and #inv.equipslots) or "nil",
            equipslots_keys = {},
